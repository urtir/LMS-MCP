...
class SecurityReportGenerator:
    ...
    def __init__(self):
        self.wazuh_db_path = TelegramBotConfig.DATABASE_CONFIG['wazuh_db']
        self.chat_db = ChatDatabase()
        self.mcp_bridge = FastMCPBridge()
        
        # Initialize LM Studio client (same as existing webapp)
        self.llm_client = OpenAI(
            base_url=TelegramBotConfig.LM_STUDIO_CONFIG['base_url'],
            api_key=TelegramBotConfig.LM_STUDIO_CONFIG['api_key']
        )
        self.model = TelegramBotConfig.LM_STUDIO_CONFIG['model']
